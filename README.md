...
> 다른 언어보다 더 재밌었다. 학교 생활 하면서 어려가지 프로젝트를 해봤지만 이렇게 디테일을 실제 모델과 비슷하게 개발하려 노력한 것은 이번이 처음이다. 그리고 원래 새로운 기술이나
> 라이브러리에 두려움이 있어 알고 있는 코드지식으로만 프로그램을 만들었었는데 이번에 새로운 것에 도전해보고 싶어 여러가지를 사용해봤다. (사실상 별거아닌 JSON, XML, 그 외 라이브러리..)
> 별거 아닌 것들이지만 새로움을 사용함에 있어 어렵지 않다 느꼈고 역시 쓰는덴 이유가 있구나 할 정도로 너무 좋았다. 사실 깃허브도 이번에 처음 계정을 만들고 사용했다.. 이 것도 짱
> 이번을 계기로 새로운 기술이나 라이브러리에 관심이 엄청 생기며 공부하고 싶은 생각이 드는 프로젝트라 의미가 뜻 깊다. 개발자로써의 첫 걸음이다.
